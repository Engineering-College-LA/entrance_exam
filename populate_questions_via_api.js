...
async function run() {
  console.log('Populating questions via API...');
  
  // Verify table has no questions first
  const { data: existing, error: checkError } = await supabase.from('questions').select('id');
  if (checkError) {
    console.error('Failed to query questions table. Does it exist?', checkError.message);
    return;
  }
  
  if (existing && existing.length > 0) {
    console.log(`Table already has ${existing.length} questions. Skipping population.`);
    return;
  }
  ...
  if (insertError) {
    ...
    console.log(`🎉 SUCCESS! Populated ${items.length} questions into Supabase!`);
  }
}
